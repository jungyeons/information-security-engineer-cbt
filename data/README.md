...
- JSON 최상위 값은 객체여야 하며 `exams` 배열에 회차가 하나 이상 있어야 합니다.
- 각 회차에는 문항이 하나 이상 있어야 합니다.
- 문항의 `text`는 비어 있을 수 없습니다.
- `choices`에는 보기가 두 개 이상 있어야 합니다.
- `answer`는 1부터 보기 개수 사이의 정수여야 합니다.

형식이 맞지 않으면 화면 상단에 구체적인 가져오기 오류가 표시되며, 잘못된 데이터는 브라우저 저장소에 저장되지 않습니다.
